...
\definecolor{neonblue}{HTML}{02ccfe}
...
The value of a flow in a given network \( G = (V, E, s, t, \textbf{u}) \) is established as the sum of flows into the source vertex \( s \), denoted as \( \operatorname{val}(f) = \sum_{v:(v, s)} f_{vs} \), and a maximum flow is identified as a feasible flow with the highest attainable value.
...
\begin{algorithm}[H]
	\scriptsize
\begin{algorithmic}[0]
	\Require $\mathcal{A}_{L\times T \times T}$: A tensor containing the aggregated attentions.
	\Ensure Tuple: $(\mathcal{F}, \widetilde{\mathcal{F}})$
	\Function{get\_attention\_flow}{$\mathcal{A}$}
	\State $L, T, \text{$\_$} \gets$ \text{$\mathcal{A}$.shape()}
	\State $\text{$\beta_{\min}$} \gets \min(\mathcal{A}>0)$
	\State $\text{$\beta$} \gets -\lfloor \log_{10}(\text{$\beta_{\min}$}) \rfloor$
	\State $\gamma \gets 10^\beta$
	\State $\mathcal{\mathcal{F}} \gets \text{np.zeros}((L + 1) \ast T + 2, (L + 1) \ast T + 2)$
...
%	\Comment{Source and Sink: infinity capacity}
	...
	\EndFor
	\For{$\text{$\ell$}$ \text{ in range}$(0, L)$}
	\State $\mathcal{F}[T \ast (\text{$\ell$} + 1) + 1 : T \ast (\text{$\ell$} + 2) + 1, T \ast (\text{$\ell$}) + 1 : T \ast (\text{$\ell$} + 1) + 1] \gets \mathcal{A}[\text{$\ell$}]$
	\EndFor
	\State $\widetilde{\mathcal{F}} \gets\text{int}(\gamma \ast \mathcal{F})$
	\EndFunction
\end{algorithmic}
\caption{Get Attention Flow-Backward Approach}\label{algo:1}
\end{algorithm}
\end{minipage}
...
%		\Comment{Source and Sink: infinity capacity}
		...
		\EndFunction
	\end{algorithmic}
	\caption{Get Attention Flow-Forward Approach}\label{algo:2}
\end{algorithm}
\end{minipage}
\adddetail{check the algorthms accuracy add source and node indices to algorithms}{Behrooz}
